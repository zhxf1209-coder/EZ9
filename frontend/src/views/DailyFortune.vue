<template>
  <div class="max-w-5xl mx-auto">
    ...
    <div class="text-center mb-8">
      <h2 class="chinese-title text-3xl md:text-4xl mb-3">阴阳流转·今日吉凶</h2>
      <p class="text-china-brown/80 text-base md:text-lg leading-relaxed max-w-2xl mx-auto">
        把握今日时机，顺势而为。每日运势动态更新，助您趋利避害，把握每一个有利时机，在合适的时间做合适的事情。
      </p>
    </div>
    ...
    <div class="chinese-card p-6 md:p-8">
      ...
      <button @click="analyze" :disabled="!canAnalyze" class="w-full chinese-btn mt-6 text-lg py-3">
        {{ loading ? '阴阳推演中...' : '阴阳流转·详批今日' }}
      </button>
    </div>

    <div v-if="result" class="mt-8 chinese-card p-6">
      <div class="text-center mb-6">
        <div class="text-lg text-china-brown">{{ todayInfo.date }}</div>
        <div class="text-2xl font-bold text-china-red mt-2">{{ todayInfo.ganZhi }}</div>
      </div>
...
const result = ref('')
const todayInfo = ref({ date: '', ganZhi: '' })
...
const analyze = async () => {
  ...
  try {
    ...
    todayInfo.value.date = `${today.getFullYear()}年${today.getMonth() + 1}月${today.getDate()}日`
    ...
    const res = await api.post('/ai/interpret', requestData)
    result.value = res.data.content
    todayInfo.value.ganZhi = res.data.bazi?.fourPillars?.day || ''
